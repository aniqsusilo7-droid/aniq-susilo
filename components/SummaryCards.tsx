...
const SummaryCards: React.FC<SummaryCardsProps> = ({ income, totalBudget, totalActual }) => {
  ...
  const cards = [
    ...
    },
    {
      ...
      value: totalBudget,
      icon: <Scale size={18} />,
      color: 'bg-slate-800 text-sky-400',
      hasUtilization: true,
    },
    {
      title: 'Pengeluaran',
    ...
    },
    {
      ...
      color: balance < 0 ? 'bg-rose-500/20 text-rose-400' : 'bg-indigo-500/20 text-indigo-400',
    },
  ];
  ...
  return (
    <div className="grid grid-cols-2 gap-4">
      {cards.map((card, i) => (
        <div 
          key={i} 
          className="bg-slate-800/40 p-5 rounded-[24px] border border-slate-700/50 shadow-lg flex flex-col justify-between"
        >
          <div>
